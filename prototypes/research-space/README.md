...
- 데이터는 브라우저 메모리의 합성 예시다.
- 실제 ORACC/CDLI 데이터와 연결하지 않는다.
- Context Package JSON을 아직 직접 읽고 쓰지 않는다.
- 실제 LLM 호출이 없다.
- 3D Canvas는 구현하지 않는다.

## 다음 연결점

다음 단계는 `schemas/context-package.schema.json`과 Research Space 노드 모델 사이의 변환 계약을 정의하는 것이다.

권장 순서:

1. Context Package → Research Space read-only adapter
2. Evidence / Interpretation / Debate / Uncertainty 시각적 구분
3. 사용자 승인 이벤트 모델
4. 수정된 Research Space → 검토용 Context Package delta
5. 실제 연구 질문 1개를 end-to-end로 실행
